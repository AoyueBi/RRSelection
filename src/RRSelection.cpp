...
void  RRSelection_help()
{
	cout <<""
		...
		"\t\t\n"
		"\t\t-help                 See more help [hewm2008 Beta v0.81]\n"
		"\n";
}
...
int main(int argc, char *argv[])
{
	...
	(paraFA04->Masked)=int((paraFA04->Masked)*((paraFA04->Masked)-1)/2);
	...
	if ( IFTwoGroup ==1 ) // run one group 
	{
		...
		SilidingRRCal(paraFA04, para_18 , SNPList , MaxSNPSite );
	}
	else if ( IFTwoGroup ==2 )// run two group
	{
		cout<<"##begin pair-wise R^2 cal. after filter Remain SNP Number :\t"<<Flag_for_pro<<"\n#\% windows bin is\t"<<(paraFA04->WindowSize)<<endl;
		SilidingRRCalTowGroup(paraFA04, para_18 , SNPList , SNPListSub , MaxSNPSite , GroupID );
	}
